...
var trueColor432Vis = {
  ...
  var params = {
        ...
        'asset_rois_soil':  "projects/nexgenmap/MapBiomas2/LANDSAT/DEGRADACAO/ROIsSoilexposto",
...
function getPointsfromFolderManual(idreg, nyear){
    print(" região " + idreg + " => " + String(nyear));
    var getlistPtos = ee.data.listAssets(params.asset_rois_soil);   
    var featColsPtos = ee.FeatureCollection([]);
    print("getlistPtos ", getlistPtos.assets);
    getlistPtos.assets.forEach(function(idAsset){
        // # print(" dict >> ", idAsset['id'])     
        var path_ = idAsset.id;
        var partes = path_.split('/');
        var name_file = partes[partes.length - 1];
        // print(name_file);
        if ((name_file.indexOf(String(idreg)) !== -1) & (name_file.indexOf(String(nyear)) !== -1)){
            print("loading and merge .... featureCollection >>> " + name_file);
            partes = name_file.split('/');
            var mes =  partes[partes.length - 1];
            var feat_tmp = ee.FeatureCollection(path_);
            feat_tmp= feat_tmp.map(
                          function(feat){ 
                              return feat.set('month', parseInt(mes));
                          })
...
function index_select(nImg, reescalar){
    ...
    return imgM.addBands(ndvi).addBands(evi).addBands(ndsi).addBands(bsi)
            .addBands(msavi).addBands(ui).addBands(vdvi).addBands(ndbi)
            .addBands(mbi) .toUint16().copyProperties(nImg);
}
...
var bandasInt = [
        'blue', 'red', 'nir', 'swir1', 'ndvi', 'ndsi', 'ui', 'mbi', 'vdvi', 
        'ndbi', 'bsi', 'gv', 'shade', 'soil', 'ndfia', 'class'
];
var regionSelect = 21;
...
var shp_ROIs = getPointsfromFolderManual(regionSelect, yyear);
print('show metadata rois points ', shp_ROIs.limit(10));
print("dict de class ",shp_ROIs.aggregate_histogram('class'));
shp_ROIs = ( shp_ROIs
                .filter(ee.Filter.neq('class', 5))
                .remap([1,2,3], [1,0,2], 'class')
        )

var sizeROIs = shp_ROIs.aggregate_histogram('class');
print("============== first size conditions = ", sizeROIs);

var classifierGTB = ee.Classifier.smileGradientTreeBoost(params['pmtGTB']).train(shp_ROIs, 'class', bandasInt);
var classifiedGTB = mosaicMensal.classify(classifierGTB, 'classification_' + String(yyear));
...
Map.addLayer(classifiedGTB.selfMask(), {min: 0, max: 1}, 'map soil soil');
