...
params = {
    ...
    'folder_rois': {'id':'projects/nexgenmap/MapBiomas2/LANDSAT/DEGRADACAO/ROIsSoil'},
...
def findExistencia(nameFeat, lstgrades, nyears):
    for nameG in lstgrades:
        if nameG in nameFeat and str(nyears) in nameFeat:
            return True
    return False

def GetPolygonsfromFolder(assetROIs, vizinhosName, myGeom, mYear, filtrar):
    ...
    for idAsset in tqdm(getlistPtos):         
        ...
        pertence = findExistencia(name, vizinhosName, mYear)
        if pertence:
            # print(cc, " Loading path ", path_);
            FeatTemp = ee.FeatureCollection(path_).filterBounds(myGeom)   
            if filtrar:
                FeatTemp = FeatTemp.filter(ee.Filter.eq('class', 1))
            # yyear = name.split('_')[4]
            # print("set YEAR ", yyear)
            # FeatTemp = FeatTemp.map(lambda feat: feat.set('year', yyear))
...
for wrsP in params['lsPath'][biome_act][:]:   #    
    ...
    for wrsR in params['lsGrade'][biome_act][wrsP][:]:   #
        ...
        gradesViz = gradeLandsat.filterBounds(geoms.buffer(200))        
        print(gradesViz.size().getInfo())
        lst_pathRow = gradesViz.reduceColumns(ee.Reducer.toList(2), ['PATH','ROW']).get('list').getInfo()
        lstVizinhos = []
        for ppath, rrow in lst_pathRow:
            print(" ", ppath, " ", rrow)
            lstVizinhos.append(str(ppath) + "_" + str(rrow))
        ...
        for index, ano in enumerate(lsAnos):
            ...
            # "rois_fitted_image_PANTANAL_1995_227_74_Soil"
            roisSoils = GetPolygonsfromFolder(params['folder_rois'], lstVizinhos, geoms.buffer(3500), ano, True)
            roisVegs = GetPolygonsfromFolder(params['folder_roisVeg'], lstVizinhos, geoms.buffer(3500), ano, False)
