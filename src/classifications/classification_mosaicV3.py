...
params = {
    ...
    'folder_rois': 'projects/nexgenmap/MapBiomas2/LANDSAT/DEGRADACAO/ROIsInt3CC',   
    ...
    'lsPath' : {
        'AMAZONIA' : [
            ...
            '227','228','229','230','231','232','233''225','226', #  
        ], # 
...
for wrsP in params['lsPath'][biome_act][:]:   #    
    ...
    for wrsR in params['lsGrade'][biome_act][wrsP][:]:   #
        ...
        for index, ano in enumerate(lsAnos):            
            ...
            # myreg = biome_act + str(ano) + '_' + regpath
            if keyPathRow in lstParPathRowViz:
                roisSoils = GetSHPsfromVizinhos(dictPathRowVis[keyPathRow], geoms, ano, biome_act)      
                sizeROIs = roisSoils.size().getInfo() 
                print("SIZE ROIs =", sizeROIs)
                
            else:
                sizeROIs = 0
                roisSoils = ee.FeatureCollection([])
            ...
            if sizeROIs > 0 or sizeROIsM > 0:            
                # print("rois aggregate ", roisSoils.aggregate_histogram('class').getInfo())
                if sizeROIsM > 0:
                    roisSoils = roisSoils.merge(ROIsManual)
                try:
