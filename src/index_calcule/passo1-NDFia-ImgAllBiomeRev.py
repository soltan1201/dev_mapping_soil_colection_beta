...
class ClassMosaics(object):
    ...
    def addDependents (self, image):
        ...
        years = image.date().difference(self.options['initYear'], 'day')
        timeRadians = ee.Image(years.multiply(2 * math.pi)).rename('t')

        constant = ee.Image.constant(1).toFloat()
...
print(lsAnos)
dict_index = {}
for cc, yy in enumerate(range(params['initYear'], params['endYear'])):
    dict_index[str(yy)] = cc
...
# Capturados aquelas cenas que falharam no processamento
tilesAnosFails = dictitemFails[biome_act]

for cc, cena in enumerate(tilesAnosFails):
    # cont = gerenciador(cont, params)
    if cc < 3:
        print(f'processing ==>> {cena} <<===' )
        # fitted_image_AMAZONIA_1994_222_61
        partes = cena.split('_')
        wrsP = partes[-2]
        wrsR = partes[-1]
        yyear = int(partes[-3])
        ...
        if (yyear - 1985) < 3:
            ...
            for oAno in lsWindowYear[dict_index[str(yyear)]]:                                    
                ...
                if colSize > 1:
                    ...
                    print("numero de imagens  Coleção Landsat ", colecaoLandsat.size().getInfo()) 
                    print(ee.Image(colecaoLandsat.first()).bandNames().getInfo())      
                    ...
                    print("numero de imagens  mistura espectral ", colecaoLandsat.size().getInfo())   
                    print(ee.Image(colecaoLandsat.first()).bandNames().getInfo())
                    harmonicLandsat = colecaoLandsat.map(lambda img: myclassMosaico.addDependents(img))
                    
                    print("harmonicLandsat size ", harmonicLandsat.size().getInfo())
                    print(ee.Image(harmonicLandsat.first()).bandNames().getInfo())
