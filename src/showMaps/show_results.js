...
var param = {
    ...
    'caatinga': 'projects/mapbiomas-arida/doctorate/soil_random_forest_caatinga',
...
var mapsSoils = null;
var mapsSoilsV2 = null;

if (bioma_select == 'CAATINGA'){
    mapsSoils = ee.ImageCollection(param.caatinga)
                        .filter(ee.Filter.eq('year', yyear)).first();

    mapsSoilsV2 = ee.ImageCollection(param.caatinga)
                        .filter(ee.Filter.eq('year', yyear)).first()
}else{
    ...
    var mapsSoilsV2 = ee.ImageCollection(param.classmappingV2).filterBounds(regioes)
                        .filter(ee.Filter.eq('year', yyear)).max();
                        // print(mapsSoils);
}
...
var maskVersion2 = mapsSoilsV2.eq(1);
