...
function GET_NDFIA_mosaic(IMAGE) {
    ...
    var lstFractions = ['gv',  'npv', 'soil', 'shade', 'cloud'];
    var endmembers = [            
...
function export_tabled(rois_exp, name_export){
   
    var param_export = {
            collection: rois_exp, 
            description: name_export, 
            folder: 'rois_coleta_soilpoints'
        }
    Export.table.toDrive(param_export);    
...
var maskYYSoiljoined = layer_soil.add(areas_col_notSoil)   //# .add(areas_col_transic)
var MosaicoClean = (mosaicMensalGEE.toUint16().addBands(maskYYSoiljoined.rename('class'))
                                .select(bandasInt)
                                .updateMask(maskYYSoiljoined.gt(0))
                                .selfMask()
                                )

var ptosTemp = MosaicoClean.stratifiedSample({
                                        numPoints: 500, 
                                        classBand: 'class', 
...
export_tabled(ptosTemp, 'pontos_coletados');
...
var lstBNB = [
  'ndvi', 'ndsi', 'bsi', 'msavi', 'ui', 'mbi', 'soil', 'cloud', 'shade',
  'vdvi', 'ndbi','evi', 'bi', 'hbsi','miim','ibi'
]

lstBNB.forEach(function(bnd){
    Map.addLayer(mosaicMensalGEE.select(bnd), paramVis.indiceNDFI, bnd + '_index', false);
})
