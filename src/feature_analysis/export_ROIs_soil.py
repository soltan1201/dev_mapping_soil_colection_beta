...
lst_Biome = ['MATA_ATLANTICA','PAMPA','PANTANAL'] # ,'CERRADO'
# lst_Biome = ['AMAZONIA','CAATINGA','CERRADO','MATA_ATLANTICA','PAMPA','PANTANAL'] #
biome_act = 'AMAZONIA'
cont = 30
            ...
            for index, ano in enumerate(lsAnos):
                ...
                if nameROIs not in lstFeatCols:
                    ...
                    if len(bandsIm) > 1:
                        ...
                        reMapbiomasYY = mapsBiomas.select('classification_' + str(yyear)
                                                ).remap(params['classMapB'], params['classNew'])    

                        maskMapbiomas  = reMapbiomasYY.eq(1)              
                        if coletaSoil:
                        ...
                        imgMosYY = imgMosYY.select(lstBnd).updateMask(maskMapbiomas.eq(0))
                        pmtSample= {
                            'numPoints': 1500,
                            'classBand': 'class',
                            'region': geomsBounds,                            
