...
def set_prop_date_Image_Harm(img):
    nband = ee.Algorithms.String(ee.List(img.bandNames()).get(0));
    lname = nband.length();
    yyear = ee.Number.parse(nband.slice(ee.Number(lname).subtract(8), ee.Number(lname).subtract(4)));
    mmonth = ee.Number.parse(nband.slice(ee.Number(lname).subtract(4), ee.Number(lname).subtract(2)));
    dday = ee.Number.parse(nband.slice(ee.Number(lname).subtract(2), ee.Number(lname)));
    dataImg = ee.Date.fromYMD(yyear, mmonth, dday);
    return ee.Image(img).rename('class').set('system:time_start', dataImg);


def convert_bandIndexs_to_Image(img):
    nband = ee.Algorithms.String(ee.List(img.bandNames()).get(0));
    lname = nband.length();
    yyear = ee.Number.parse(nband.slice(ee.Number(lname).subtract(6), ee.Number(lname).subtract(2)));
    mmonth = ee.Number.parse(nband.slice(ee.Number(lname).subtract(2), ee.Number(lname)));    
    dataImg = ee.Date.fromYMD(yyear, mmonth, 1);
    return ee.Image(img).rename('class').set('system:time_start', dataImg);


def convert_band_imgto_Image(images):
    redBnds = ee.List(images.bandNames()); 
    def process_lst_bnds(itmbnd, mylist):
        img = ee.Image(images).select([itmbnd]);
        lname = ee.Algorithms.String(itmbnd).length();
        subStringbnd = ee.String(itmbnd).slice(ee.Number(lname).subtract(2), ee.Number(lname).subtract(1));
        lstUpdate = ee.Algorithms.If( 
                        ee.Algorithms.IsEqual(subStringbnd.compareTo(ee.Algorithms.String('_')), 0),
                        ee.List(mylist),
                        ee.List(mylist).add(set_prop_date_Image_Harm(img))
                    );
        return lstUpdate;
    
    lstIterImg = redBnds.iterate(process_lst_bnds, ee.List([]) );
    return ee.ImageCollection(lstIterImg)

def convert_indexBands_to_indexImg(img):
    redBnds = ee.List(images.bandNames());  
    def process_lst_bnds(itmbnd, mlist):
        img = ee.Image(images).select([itmbnd]);
        lstUpdate = ee.List(mlist).add(convert_bandIndexs_to_Image(img));
        return lstUpdate;
...
param = {
    ...
    'lsGrade' : {
        ...
        'CERRADO' : {
           ...
           '219' : ['70','71','72','73','74','75','76'], #  '62','63','64','65','66','67','68','69',		
...
# sys.exit()
cont = gerenciador(16, param) 
gradeLandsat = ee.FeatureCollection(param['gradeLandsat'])

for wrsP in param['lsPath'][biome_act][:2]:   #    
    ...
    for wrsR in param['lsGrade'][biome_act][wrsP][:]:   #
        ...
        for cc, yyear in enumerate(lsAnos):
            ...
            size_featROIsSoil = 0
            size_featROIsVeg = 0
            roisPathRow = ee.FeatureCollection([])
            try: 
            ...
            try:
                if keyPathRow in lstKeysVeg:
                    ...
                    print(" colect Ptos in == ", path_asset_veg)
            ...
            print(mosaicHarms.bandNames().getInfo())
            ...
            print(mosaicNDFIaIndex.bandNames().getInfo())

            if size_featROIsSoil > 0 or size_featROIsVeg > 0:
                
                # lstHarmBands  = convert_band_imgto_Image(mosaicHarms)                
                # print("we separate = ", ee.List(lstHarmBands).length().getInfo())                
                ...
                roisPathRow = roisPathRow.merge(points)        

                #  addiding the same ptos in the other mosaic 
                pointsNDFIa = mosaicNDFIaIndex.sampleRegions(**pmtSample)
                roisPathRow = roisPathRow.merge(pointsNDFIa)

            nameROIsExp = f"rois_{biome_act}_{yyear}_{wrsP}_{wrsR}"
            processoExportar(roisPathRow, nameROIsExp)
